...
function CommentDetail() {
  ...
  const handleDelete = () => {
    commentService.bulkDelete([id]).then(res => {
      if (res.status = 'update') {
        Alert.showDeleteSuccess();
        router.push(routes.find((r) => r.idx == "comment").path);
  ...
  if (comment.listPhoto != null) {
    comment.listPhoto = comment.listPhoto.replace('[', '');
    comment.listPhoto = comment.listPhoto.replace(']', '');
    console.log("ẢNH ĐÃ CÓ" + comment.listPhoto);
    if (comment.listPhoto != '' && (comment.listPhoto.split(",")).length > 0) {
                  ...
                  <Row className="align-items-center">
                    ...
                    <Col className="text-right">
                      <Button
                        ...
                        type="button"
                        onClick={() => { setModalOpen(!modalOpen); console.log("modl") }}
                      >
                        <i
                  ...
                  <div className="pl-lg-4">
                    <Row className="align-items-center">
                      ...
                      <div className="col ml-2">
                        <h4 className="mb-0">
                          <Link href={"/diners/" + comment.user.id}>{comment.user.fullName}</Link>
                        </h4>
...
CommentDetail.layout = AdminLayout;
